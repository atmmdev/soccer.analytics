# Documentação — Soccer Analytics

Documentação do projeto organizada por módulo.

## Módulos

| Módulo | Caminho | Descrição |
|--------|---------|-----------|
| **Betting** | [betting/README.md](./betting/README.md) | Mercados, glossário, IA e exemplos de bilhetes |
| Engenharia | [.ai/](../.ai/) | Arquitetura, engines e tarefas de desenvolvimento |

### Betting (base de conhecimento)

- [README](./betting/README.md) — índice e convenções
- [Glossário](./betting/glossary.md) — termos de apostas esportivas
- [Mercados](./betting/markets/) — 10 categorias (01–10)
- [IA](./betting/ai/) — score, EV, correlações, checklist
- [Exemplos](./betting/examples/) — bilhetes modelo
