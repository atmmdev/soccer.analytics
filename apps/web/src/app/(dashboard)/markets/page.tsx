...
import { Loader2, Play, Plus, Ticket, TrendingUp } from 'lucide-react';
...
function TeamCrest({
  ...
  if (!src || failed) {
    return (
      <span
        className="flex shrink-0 items-center justify-center rounded-full bg-secondary text-xs font-semibold text-muted-foreground"
...
function Stat({
  ...
  return (
    <div className="space-y-1 text-center sm:text-left">
      ...
      <p className={`font-mono text-sm font-medium ${muted ? 'text-muted-foreground' : ''}`}>
        {value}
      </p>
    </div>
  );
...
export default function MarketsPage() {
  ...
  const [ticketProfile, setTicketProfile] = useState<TicketProfileId>('moderado');
  ...
  const filteredMarkets = useMemo(() => {
    if (competition === ALL_COMPETITIONS) return markets ?? [];
  ...
  useEffect(() => {
    ...
    if (!selectedMatchId || !matchGroups.some((g) => g.matchId === selectedMatchId)) {
      setSelectedMatchId(matchGroups[0].matchId);
    }
  ...
  const homeLogo =
    ...
    selectedGroup?.homeLogoUrl ??
    null;
  const awayLogo =
    matchDetail?.awayTeam.logoUrl ??
    selectedGroup?.awayLogoUrl ??
    null;
      ...
      <div className="flex-1 space-y-4 p-6">
        <div className="flex flex-wrap items-end gap-3">
          <div className="w-full max-w-xs space-y-2">
            <label htmlFor="competition-select" className="block text-sm font-medium">
              Campeonato
            </label>
            <Select
              value={competition}
            ...
            >
              <SelectTrigger id="competition-select" className="h-10 bg-secondary/30">
                <SelectValue placeholder="Selecionar campeonato" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={ALL_COMPETITIONS}>Todos</SelectItem>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="mx-auto max-w-2xl space-y-6">
                ...
                <div className="flex flex-col items-center border-b border-border/40 pb-5 text-center">
                  <div className="flex items-center justify-center gap-4 sm:gap-6">
                    <div className="flex w-24 flex-col items-center gap-2 sm:w-28">
                      <TeamCrest src={homeLogo} name={homeTeam} size={56} />
                    ...
                    <div className="flex w-24 flex-col items-center gap-2 sm:w-28">
                      <TeamCrest src={awayLogo} name={awayTeam} size={56} />
                ...
                {/* Bilhetes sugeridos por perfil (docs/betting/examples) */}
                {activeTicket && (
                    ...
                    <div className="mb-3 flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                          Exemplos montados com os mercados analisados de{' '}
                          {homeTeam} vs {awayTeam}, seguindo os perfis de{' '}
                          <code className="text-[10px]">docs/betting/examples/</code>.
                        </p>
                      </div>
                    </div>
                    ...
                    {!activeTicket.buildable ? (
                      ...
                      <>
                          ...
                          {activeTicket.legs.map((leg, index) => (
                            ...
                            >
                              <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
                                ...
                                <Badge variant={RECOMMENDATION_VARIANT[leg.recommendation]}>
                              ...
                              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-mono text-muted-foreground">
                                ...
                                <span>Prob. {(leg.probability * 100).toFixed(1)}%</span>
                                <span>
                                  EV {leg.ev >= 0 ? '+' : ''}
                        ...
                        <div className="mb-3 grid grid-cols-2 gap-3 rounded-md bg-secondary/30 p-3 text-sm sm:grid-cols-4">
                          <div>
                            <p className="text-[11px] text-muted-foreground">Pernas</p>
                            <p className="font-medium">{activeTicket.legs.length}</p>
                          </div>
                          <div>
                            <p className="text-[11px] text-muted-foreground">Odd combinada</p>
                            <p className="font-mono font-medium">
                              {activeTicket.combinedOdd.toFixed(2)}
                            </p>
                          </div>
                          <div>
                            <p className="text-[11px] text-muted-foreground">EV médio</p>
                            <p className="font-mono font-medium">
                          ...
                          <div>
                            ...
                            <p className="font-mono font-medium">
                              R$ {stakeExampleReturn(activeTicket.combinedOdd).toFixed(2)}
                            </p>
                          </div>
                        </div>
